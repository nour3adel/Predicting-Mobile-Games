...
class DataPreprocessing_Test:
    ...
    def preprocess_Average_User_Rating(self):
        avg_nan = self.dataset["Average User Rating"].isnull().sum()
        AUR_mean = self.dataset['Average User Rating'].mean()
        if avg_nan != 0:
            self.dataset['Average User Rating'] = self.dataset['Average User Rating'].fillna(AUR_mean)
    ...
    def DataScaling(self):
        x_test = self.dataset.drop(columns = ['Average User Rating'])  # Features   
        y_test =  self.dataset['Average User Rating']  # Label
    ...
    def preprocess_all(self):
        ...
        self.preprocess_Average_User_Rating()
